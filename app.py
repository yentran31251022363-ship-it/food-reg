...
def load_deep_model():
    try:
        ...
        st.error(f"❌ Cảnh báo hệ thống: Không thể tải file model! Lỗi chi tiết: {e}")
        return None
...
with col_left:
    ...
    if uploaded_file is not None:
        ...
        try:
            ...
            st.error(f"Không thể tải ảnh mẫu từ internet: {e}")
...
with col_right:
    ...
    if active_image is not None:
        if model is None:
            st.warning("⚠️ Ứng dụng hiện chưa nạp được mô hình AI. Vui lòng kiểm tra file trọng số (.keras) trên GitHub.")
        else:
            t_start = time.time()
